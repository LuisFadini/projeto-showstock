...
export class VendedoresController {
  ...
  paginaListar(req: Request, res: Response) {
    const nome = req.params.nome;
    if (!nome || typeof nome !== "string")
      return res.status(400).send("Precisa ser fornecido o nome do vendedor");
  ...
  paginaEditarProduto(req: Request, res: Response) {
    ...
    const produto = comerciante.produtos.find((p) => p.id === Number(id));
  ...
  editarProduto(req: Request, res: Response) {
    const { nome, id } = req.params;
  ...
  deletarProduto(req: Request, res: Response) {
    ...
    const index = comerciante.produtos.findIndex((p) => p.id === Number(id));

    if (index === -1) {
      return res.status(404).send("Produto não encontrado");
    }

    const [produtoRemovido] = comerciante.produtos.splice(index, 1);

    return res.json(produtoRemovido);
  }
}
